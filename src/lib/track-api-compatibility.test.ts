...
describe('track API compatibility guard', () => {
  ...
  it('accepts a valid 1.2 definition with asymmetric zones and an optional fence', async () => {
  ...
  ])('rejects a nominal 1.2 definition with $label', async ({ mutate }) => {
  ...
  it('rejects a nominal 1.2 catalog from another catalog generation', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        jsonResponse({
