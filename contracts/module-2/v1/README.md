...
O catálogo ativo `2026.5` preserva a geometria, superfícies e proteções validadas de `2026.4` e substitui os antigos marcos genéricos por perfis explícitos dos 24 circuitos. Cada lado de cada trecho contém zonas ordenadas de asfalto, grama ou brita, uma barreira física de impacto e, quando documentada, uma `debris-fence` externa independente. A colisão ocorre na barreira após a soma das larguras das zonas; a grade é somente uma segunda camada visual. A centerline fechada é suavizada e reamostrada a cada aproximadamente 5 m; `halfWidthMeters` acompanha variações sustentadas pelas fontes e `elevationLayer` separa níveis sobrepostos. `curbs[]` publica zebras por intervalo, lado, largura, cadência e paleta. `sceneryLayout` publica tipos semânticos, posições, rotações e escalas ancorados a frações da mesma centerline. Os perfis são aproximações auditadas e referenciadas para gameplay; a fonte da centerline continua não sendo um levantamento topográfico de muros e áreas de escape.
