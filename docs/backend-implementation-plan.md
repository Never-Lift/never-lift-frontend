...
- Toda mutação do lobby transmite `room_state` imediatamente. `ready` é reversível e não bloqueia os ajustes nem é limpo quando o host os altera; as configurações travam somente no início da classificação. Host e participantes comuns podem sair explicitamente em qualquer fase, com transferência automática do host, enquanto uma queda preserva o participante por 30 s para reconexão antes da remoção automática.
