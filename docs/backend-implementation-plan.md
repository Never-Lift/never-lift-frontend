...
**Decisão de simplificação aprovada em 24/08/2026 (backend #72 / frontend #90):** o produto passa a ter somente o F1 e uma configuração fixa de condução baseada nos valores atuais do perfil Normal. `carModel`, `handlingMode`/`driftMode`, os perfis Supercarro/Drift e as dimensões de recorde associadas serão removidos. A implementação exige uma revisão física incompatível `1.3.0`, sincronizada nos dois repositórios. Até essa implementação ser aprovada e concluída, os JSON executáveis `1.2.0` permanecem canônicos para preservar o sistema validado.
