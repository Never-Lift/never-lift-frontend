...
**Contrato de entrada:** `contracts/module-2/v1/` define `TrackDefinition v1`, catálogo `2026.1`, constantes físicas `1.2.0` e as decisões fechadas do modo local. O frontend consome o catálogo pela API; não mantém uma segunda cópia das 24 geometrias em produção.
**Estado da entrega:** em andamento. As Partes 2a e 2b estão concluídas e validadas: motor e regras compartilhadas v1.2, catálogo e geometrias oficiais, câmera dinâmica, minimapa, split-screen e culling por chunks. A Parte 2c (ambiente, iluminação e largada) permanece pendente. Este estado parcial não torna o Módulo 2 pronto.
...
**Parte 2b entregue:** `GET /api/tracks` e `GET /api/tracks/{id}` tipados pelo contrato real; preparação com as 24 pistas, metadados, ambiente e prévia; `TrackDefinition` injetada no `RaceEngine`; grid, largura, barreiras, superfícies, checkpoints direcionais e `racingLine` oficiais; câmera por jogador suavizada em `0,25s`, escala de carro em `5,5%` da altura e enquadramento em `60%`; minimapa fixo; split vertical ou horizontal no limite `1,35`; e renderização vetorial somente dos `chunks` visíveis. O resultado usa o `id` e a `catalogVersion` selecionados. Monaco e Spa-Francorchamps foram percorridos integralmente com bots, e a interface real foi verificada nos modos solo e local.
