...
**Estado da entrega:** em andamento. A Parte 2a (motor) está implementada; a correção de dano mecânico v1.1 aguarda revalidação manual na preview. As Partes 2b (24 pistas e geometrias da API) e 2c (ambiente e polimento) permanecem pendentes. Este estado parcial não torna o Módulo 2 pronto.

**Parte 2a entregue:** `RaceEngine` determinístico a `1/60s`, interpolação entre ticks, perfis e superfícies do contrato v1.1, alternância normal/drift, colisões de carros e barreiras, dano mecânico determinístico, oval técnico temporário, seleção dos três modelos e cores, bots determinísticos, dois jogadores com controles distintos, corrida curta completa e persistência em `POST /api/races/local-result`. O motor e o Canvas mantêm posições e velocidades fora do estado React. A equivalência a 30/60/120 FPS, as regras físicas, os inputs, as colisões, os dois modos e o payload REST possuem cobertura automatizada.
