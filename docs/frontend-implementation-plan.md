...
**Contrato de entrada atual:** `contracts/module-2/v2/` define `TrackDefinition` `2.0.0`, catálogo `2026.9`, constantes físicas `2.0.0`, colliders compostos, faces canônicas de barreira e perfis visuais métricos de infraestrutura. O frontend consome as 24 geometrias pela API e mantém localmente somente os artefatos comuns do contrato; `contracts/module-2/v1/` preserva o runtime `1.3.0` como histórico imutável.
**Estado da entrega:** em andamento. As Partes 2a, 2b e 2c e o catálogo `2026.5` passaram pela validação manual integrada em 24/08/2026. A simplificação para F1 único/condução única e o refinamento de câmera 2.5D/F1 multidirecional aguardam validação manual final no preview. Na Parte 2d, a implementação automatizada está concluída. A revisão de fidelidade `2026.9` das 24 pistas também está automatizada, com zebras autorais por curva, pits, construções e cercas em medidas de mundo, auditoria de sobreposição, composição lisa do cruzamento de Suzuka e via visual de escape em slalom no Rettifilo ainda exigindo validação manual; a Parte 2d e o Módulo 2 não estão prontos.
