...
**Contrato de entrada:** `contracts/module-2/v1/` define `TrackDefinition` `1.1.0`, catálogo `2026.2`, constantes físicas `1.2.0` e as decisões fechadas do modo local. O frontend consome o catálogo pela API; não mantém uma segunda cópia das 24 geometrias em produção.
**Estado da entrega:** em andamento. A Parte 2a está concluída e validada. A Parte 2b está implementada e recebeu uma correção de validação para limites segmentados, bots em curvas estreitas e câmera em ré; aguarda revalidação manual. A Parte 2c (ambiente, iluminação e largada) permanece pendente. Este estado parcial não torna o Módulo 2 pronto.
...
- Catálogo `2026.2` congelado com as 24 etapas do calendário original de 2026, incluindo Bahrain e Jeddah mesmo diante de alterações posteriores no calendário real. Os traçados são reconhecíveis, têm comprimentos variados e escala métrica aproximada. `trackLimits` cobre toda a volta por lado: `barrier` junto ao asfalto ou `runoff` com 10 m de grama. Pequenos ajustes de 10–20% são permitidos quando necessários à jogabilidade; uma futura distribuição pública exige revisão de nomes e apresentação.
