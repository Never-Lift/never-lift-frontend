...
**Servidor → Cliente:** `room_state`, `countdown { startAtServerTime }`, `state_snapshot { tick, serverTime, trackId, trackCatalogVersion, physicsContractVersion, cars: [{ playerId, x, y, velocityX, velocityY, angle, speed, physicsState: { yawRate, steeringAngle, appliedThrottle, appliedBrake, frontWheelAngularSpeed, rearWheelAngularSpeed, gear, engineRpm, gearShiftTimeRemaining, longitudinalAcceleration }, damageState: { kind, health, engineDamaged, steeringDamaged, steeringPull, totalLoss, impactCount, lastImpactSpeed }, trackDistanceMeters, trackLayer, lastProcessedClientSeq }] }`, `race_event`, `race_result`, `error`.
...
**Otimização local de 08/09/2026:** a implementação usa o mesmo `RaceEngine`/`LocalRaceSession` em Web Worker para solo/local, mantendo física 2.0.3 a 120 Hz, contatos e comportamento. O Canvas lê snapshots locais interpolados fora do React; mensagens e memória são limitadas. Não é o transporte nem a predição online do M3c. Testes, limites de FPS, fallback, pausa de aba e validação manual pendente estão em `race-performance-worker-2026-09-08.md`. Não considerar 40 FPS mínimos aprovados apenas pela média.

**Correção de cadência/input de 10/09/2026:** eventos de teclado são enviados imediatamente, separados dos pedidos de snapshot; transições curtas são preservadas até serem consumidas por um passo da física. O relógio visual usa o timestamp do `requestAnimationFrame`, e o snapshot usa o tempo representado pela simulação, sem somar o custo de cálculo. As evidências e os limites da validação estão em `race-input-cadence-2026-09-10.md`. Não modifica o protocolo online nem inicia o M3c.

**Revisão de curvas de 11/09/2026:** substitui o uso universal de worker da rodada acima. Até dois carros totais executam o mesmo `RaceEngine`/`LocalRaceSession` diretamente no ciclo de RAF, mantendo passo físico de 120 Hz e preservação de toques curtos. De três a 22 carros, mantém-se o worker para não bloquear desenho/input com colisões congestionadas. A estratégia é fixa durante a prova, sem alterar física, design ou protocolo online. Evidências e validação manual pendente em `race-corner-stutter-2026-09-11.md`; não declara o M3c iniciado nem FPS mínimo universal.

**Colisões densas de 14/09/2026:** o autor aprovou solo/local sem bots em Spa, mas relatou pausas com 22 carros. A rodada seguinte reduz consultas geométricas e alocações nas colisões, sem mudar fórmulas, iterações, hitboxes, dano, input ou desenho. Preserva a estratégia direto/worker da rodada anterior. Diagnóstico, equivalência e limites em `race-dense-collisions-2026-09-14.md`; nova validação manual de solo 1+21 e local 2+20 pendente, sem iniciar M3c nem garantir FPS mínimo universal.
...
- Input: cada humano escolhe exatamente um dos três presets de teclado (`WASD`, setas ou `IJKL`); esquerda produz esterço à esquerda em todos os mapeamentos. No modo local os dois pilotos não podem selecionar o mesmo grupo. Rampas determinísticas permitem modular comandos digitais sem impedir perda de aderência. `Shift` não é capturado e não possui função; não existe boost, nitro ou freio de mão.
...
- Otimização de 04/09/2026: cache de geometria estática, culling de carros/pits e reaproveitamento exato de geometria de colisão, sem alterar física/bots. Local e solo com pelo menos dez carros usam perfil baixo e densidade interna máxima de 1; solo menor usa médio e máxima de 1,5. Zoom/escala permanecem fixos. Evidências, limites e validação manual pendente estão em `race-performance-2026-09-04.md`.
- Proteção de rede de 08/09/2026: o cliente deduplica carregamentos simultâneos, reutiliza o catálogo por cinco minutos e conserva somente as quatro definições mais recentes por até uma hora. Entradas rejeitadas não permanecem no cache. O backend serve esses payloads dos recursos empacotados, sem transferi-los pelo PostgreSQL.
...
publica snapshots a 20 Hz. Inputs são renovados a 30 Hz, mantidos por 200 ms e depois
neutralizados pelas rampas do contrato. A correção aprovada 2.0.2 calcula dano pelo
delta-v normal, preservando os limiares de resistência/direção de 2.0.1.

O snapshot inclui `trackId`, `trackCatalogVersion`, `physicsContractVersion`,
`tick`, `serverTime` e carros com estado completo. `physicsState` inclui também
`longitudinalAcceleration` do subpasso anterior; `damageState` inclui `kind`,
`impactCount` e `lastImpactSpeed`. Cada carro transmite `trackDistanceMeters`,
`trackLayer` e `lastProcessedClientSeq` (-1 antes do primeiro comando ou para bots).
...
**Responsabilidade:** exclusivamente frontend; não exige endpoint nem alteração no backend. Expande a seleção já existente entre os três presets fixos para remapeamento livre.
**Depende de:** Módulo 2.
**Escopo:**
