...
**Contrato de entrada:** `contracts/module-2/v1/` define `TrackDefinition` `1.3.0`, catálogo `2026.4`, constantes físicas `1.2.0` e as decisões fechadas do modo local. O frontend consome o catálogo pela API; não mantém uma segunda cópia das 24 geometrias em produção.
**Estado da entrega:** em andamento. As Partes 2a, 2b e 2c estão implementadas e a versão anterior passou pela validação manual integrada. O polimento final `2026.4` — curvas suavizadas, amostragem aproximada de 5 m, zebras contratuais e transições visuais externas — está pronto em código, atlas e testes automatizados, mas ainda exige revalidação manual integrada antes de o Módulo 2 ser marcado como pronto.
