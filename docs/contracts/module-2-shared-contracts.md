...
- nova linha `contracts/module-2/v2/`, schema de pista `2.0.0` e catálogo executável `2026.12`; o catálogo `2026.5` permanece válido somente para o runtime v1, o `2026.6` registra a publicação inicial das faces canônicas, o `2026.7` a primeira revisão estrutural, o `2026.8` a primeira rodada de infraestrutura visual, o `2026.11` as aberturas físicas de pit e as 22 vagas visuais por circuito e o `2026.12` as placas completas, as largadas corrigidas, a orientação de Marina Bay e a remoção do escape provisório de Monza.

### Revisões estruturais e visuais dos catálogos 2026.7–2026.12

A implementação automatizada está concluída e a validação manual permanece pendente. A linha preserva o contrato físico `2.0.0`, mas regenera as 24 pistas para suavizar as faces canônicas usadas simultaneamente por colisão e desenho, impedir que braços vizinhos na mesma elevação se cubram e posicionar zebras além da borda do asfalto. O `2026.9` substitui a inferência global de zebras por perfis explícitos por curva. O `2026.10` estabiliza a face interna de proteções apertadas, publica `brakingMarkers` regressivos nas aproximações relevantes e permite somente aberturas de barreira declaradas no mesmo contrato visual/físico. O `2026.11` acrescenta, em cada definição, aberturas de `pit-entry` e `pit-exit` derivadas da trajetória navegável do pit e fixa 22 vãos de garagem opacos (duas vagas para cada uma das 11 equipes), mantendo a identidade cromática e arquitetônica específica de cada autódromo. A face traseira de cada shell é publicada como `pitLane.garageBarrier` e participa da mesma colisão métrica: o pit lane fica livre, mas o carro não atravessa a garagem. O `2026.12` completa as placas solicitadas, reposiciona a largada oficial de Silverstone, orienta Marina Bay no sentido anti-horário e remove integralmente o escape provisório do Rettifilo de Monza. Landmarks provisórios dispersos permanecem fora do catálogo até a rodada futura de cenário temático.

O relatório atual, incluindo método, fontes, decisões por circuito, limitações e validações automatizadas, está em [`../module-2-track-safety-audit-2026.10.md`](../module-2-track-safety-audit-2026.10.md). Os relatórios `2026.7`–`2026.9` permanecem históricos.
...
- placas métricas de frenagem e eventuais aberturas explícitas da proteção;
