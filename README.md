...
- Durante a corrida, os nomes são mostrados na sequência de largada e, depois dela, apenas enquanto `Espaço` estiver pressionado. O HUD não usa cabeçalho: mantém telemetria periférica, logo compacta, minimapa ampliado com linha de chegada e nome do circuito, além dos controles compactos de reinício e saída no canto inferior direito. Nas corridas solo e local, o botão de reinício ou a tecla `R` recomeçam imediatamente a prova com a mesma configuração, sem confirmação; `Esc` aciona a saída disponível. Na ré, esquerda/direita preservam o lado de deslocamento pretendido pelo jogador por meio do adaptador de input, sem mudar o tuning físico.
...
- A Parte 3a mantém o lobby. A Parte 3b já executa a física no backend; classificação, largada e condução online neste cliente dependem da Parte 3c.
...
## Parte 3b — contrato 2.0.3 e motor autoritativo
...
Backend e frontend devem usar a mesma revisão 2.0.3; versões anteriores são recusadas.
