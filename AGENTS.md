...
- `docs/race-corner-stutter-2026-09-11.md` — revisão após novo relato sem bots: até dois carros usam execução direta, grids maiores mantêm worker; medições prolongadas, testes e validação manual pendente.
- `docs/race-dense-collisions-2026-09-14.md` — validação do autor em Spa sem bots (solo/local), diagnóstico de contatos congestionados e otimizações equivalentes; grids completos ainda dependem de nova validação manual.
...
> Correção de cadência visual de 09–10/09/2026: snapshots locais agora têm buffer de três quadros, movimento visual interpolado e timestamp coerente com o tempo efetivamente simulado durante catch-up. O cache de carros remotos também recicla o canvas expulso e não reorganiza o `Map` em cada hit, eliminando alocações repetidas nas curvas de grids densos sem alterar pixels nem a física 2.0.3. Gate automatizado anterior aprovado (405 testes/49 arquivos, lint, build e smoke do Edge); nova validação visual com muitos bots pendente.

> Revisão de input/cadência de 10/09/2026 (substitui a calibração do parágrafo anterior): eventos de tecla independentes de RAF/snapshot, preservação de toques curtos por piloto, snapshots datados antes do custo físico e apresentação pelo relógio do RAF. Buffer visual calibrado para 33,33 ms com até oito snapshots; tolerância de repouso do adaptador de ré em 0,1 m/s. Física 2.0.3, pistas e design preservados. Gate de 425 testes/51 arquivos aprovado; testes de navegador e limitações em `docs/race-input-cadence-2026-09-10.md`. Validação manual desta correção pendente, sem iniciar o M3c nem declarar FPS mínimo universal.

> Revisão de curvas de 11/09/2026 (substitui o uso universal de worker acima): corridas com até dois carros totais usam `LocalRaceRuntime` direto, no mesmo ciclo de RAF do desenho, sem espera por snapshots. De três a 22 carros, permanece o worker com as correções anteriores de input. Física 120 Hz/2.0.3 e design inalterados; 427 testes/51 arquivos, teclado Chrome/Edge e paridade aprovados. Medições prolongadas sem bots próximas de 60 FPS não dispensam validação manual da nova preview nem aprovam todos os grids. Ver `docs/race-corner-stutter-2026-09-11.md`; M3c permanece pendente.

> Colisões densas de 14/09/2026: o autor aprovou Spa solo/local sem bots, mas não o grid completo. Consultas geométricas e buffers privados de colisão foram otimizados preservando física 2.0.3, hitboxes, iterações, input e design. 431 testes/51 arquivos, paridade Node/Chrome/Edge, 512 casos CCD e hashes físicos de 60 segundos em Spa/Mônaco aprovados; 120 toques curtos sem perdas. As medições reduziram atraso de snapshots, mas ainda detectaram falhas de interpolação com 22 carros. Nova validação manual pendente; não declarar 40 FPS mínimos nem iniciar M3c. Ver `docs/race-dense-collisions-2026-09-14.md`.
